...
def _lattice(tmp_path, side=10, pflip=0.0):
...
@_needs_pb
def test_pb_matches_python_numerically_linear(tmp_path):
    ...
    lat = _lattice(tmp_path, pflip=0.0)
    mpy = _code(lat, runlang="py", coupling_type="linear")
    mpy.run()
    mpb = _code(lat, runlang="pb", coupling_type="linear")
    mpb.run()
...
@_needs_pb
def test_pb_product_signed_finite_and_deterministic(tmp_path):
    ...
    lat = _lattice(tmp_path, pflip=0.3)
    ...
    m1.init_ode_dynamics(custom=x0)
    m1.run()
    m2 = CoupledODEModel(lat, runlang="pb", **common)
    m2.init_ode_dynamics(custom=x0)
    m2.run()
...
@_needs_pb
def test_pb_deterministic_same_seed(tmp_path):
    # Same truthy seed -> identical initial condition -> identical pb trajectory.
    lat = _lattice(tmp_path)
    m1 = _code(lat, runlang="pb", seed=1)
    m1.run()
    m2 = _code(lat, runlang="pb", seed=1)
    m2.run()
    assert np.array_equal(m1.s, m2.s)


@_needs_pb
def test_pb_matches_python_logistic(tmp_path):
    ...
    mpy = _code(lat, runlang="py", **kw)
    mpy.run()
    mpb = _code(lat, runlang="pb", **kw)
    mpb.run()
    d = np.max(np.abs(np.asarray(mpy.s) - np.asarray(mpb.s)))
