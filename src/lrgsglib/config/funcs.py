...
def build_pT_fname(
    ...
    fname = (
        join_non_empty("_", base, peq_fstr(pflip), Teq_fstr(T), out_suffix)
        + ext
    )
    if path:
...
def build_fname_or_pattern_direct(
    ...
    match mode:
        ...
        case _:
            raise ValueError(
                f"Unsupported mode '{mode}' for filename construction."
            )
...
def find_shared_p_values(
    pattern: str, pathdir: str, extension: str = ".pkl"
...
def bin_eigenvalues(
    eigenvalues: List[float], bins: np.ndarray, bin_centers: np.ndarray
) -> Counter:
    """
    ...
    bin_indices = np.clip(
        bin_indices, 0, len(bin_centers) - 1
