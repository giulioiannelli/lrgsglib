...
phelp_entropy_steps = (
    "Number of logarithmic time points for entropy profile (τ ∈ [10^t1, 10^t2])"
)
...
phelp_entropy_seed = (
    "Random seed for entropy probe vectors (None = non-deterministic)"
)
phelp_entropy_norm = (
    "Entropy normalization: 'standard' (raw) or 'complement' (1 - S)"
)
phelp_specific_heat_scale = (
    "Specific heat scaling: 'logN' (remove size dependence) or 'none'"
)
