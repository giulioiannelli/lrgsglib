...
ONE registry key ("xy") serves BOTH the legacy ``XYModel`` and the new scheme
classes (``XYBase`` leaves): the solver polymorphs on the instance (statsys
unification D-B4), exactly as ``PottsModel/_solvers.py`` does. The
C-subprocess backend stays legacy-only (decision D-B5, frozen argv).
"""
...
def _is_new_style(model) -> bool:
    """True for the new scheme classes (XYBase & leaves); the ONE registry
    key polymorphs on the instance (D-B4)."""
    from .XYBase import XYBase

    return isinstance(model, XYBase)
    ...
    def execute(
        ...
        *,
        ...
        if _is_new_style(model):
            # New scheme classes (XYBase leaves): the compiled shared
...
class _XYCSolver:
    ...
    def supports(self, model: "XYModel") -> None:
        if _is_new_style(model):
            ...
            raise NotImplementedError(
                "The C-subprocess backend is not wired for the new XY "
...
def register_xy_solvers() -> None:
    """Register XYModel's backends (idempotent; safe to call on import)."""
