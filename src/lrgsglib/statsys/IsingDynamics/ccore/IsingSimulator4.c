...
int main(int argc, char *argv[])
{
    ...
    if (argc < EXPECTED_ARGC) {
        fprintf(stderr, "Usage: %s N T p Noclust thSTEP eqSTEP datdir syshape run_id"\
    ...
    /* check run_id */
    if (run_id[0] != '\0') {
        sprintf(modified_run_id, "_%s", run_id);  // Prepend an underscore if run_id is not empty
    } else {
        modified_run_id[0] = '\0';  // Make sure it's an empty string if run_id is empty
    }
