    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if dynpath is not None:
            dynpath = Path(dynpath) / HEISENBERG_DYN_SUBDIR
        super().__init__(
            sg,
            q=3,
            discrete=False,
    ...
    def _begin_outputs(self) -> None:
        ...
        if HEISENBERG_OBS_ENERGY in self._selected_obs:
            self.observables[HEISENBERG_OBS_ENERGY].set_path(
                self.dynpath
                / self.sg.get_p_fname(HEISENBERG_ENERGY_FBASE, suf, ext=BIN)
            )
        if HEISENBERG_OBS_MAGN in self._selected_obs:
            self.observables[HEISENBERG_OBS_MAGN].set_path(
                self.dynpath
                / self.sg.get_p_fname(HEISENBERG_MAGN_FBASE, suf, ext=BIN)
            )
        if HEISENBERG_OBS_SNAPSHOTS in self._selected_obs:
            snap.set_path(
                self.dynpath
                / self.sg.get_p_fname(HEISENBERG_SNAPSHOTS_FBASE, suf, ext=BIN)
            )
