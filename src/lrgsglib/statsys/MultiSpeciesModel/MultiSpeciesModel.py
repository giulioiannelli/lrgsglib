...
class MultiSpeciesModel(CBackendMixin, VecDynSys):
    ...
    **kw
    ...
    def _run_pybind(self) -> None:
        """Run Metropolis via the in-process ``_multispec_native`` kernel.

        Marshals the graph into the engine-agnostic CSR triple (works for NX
        and GT) and runs the same ``multispec_metropolis_sweep`` kernel the C
        subprocess uses (identity interaction, uniform q); seed-reproducible.
        State is flattened to row-major ``(N, k)`` int32 -- exactly the layout
        the C kernel indexes as ``s[nd*k + comp]``. Mirrors
        ``PottsModel._run_pybind``.
        """
    ...
    def run(
        ...
        **kw: Any,
        ...
        solver = get_solver(MULTISPEC_SOLVER_NAME, backend)
...
# Register MultiSpeciesModel's solver backends (py/pb/C) in the shared solver
# registry. Imported after the class so importing MultiSpeciesModel wires up
