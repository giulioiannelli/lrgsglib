...
def assign_random_signs(edge_list, p_flip=0.5):
    ...
    signs = {
        edge: 1 if p_flip < rand_p[i] else -1
...
def run_single_realization(
    edge_list, signs, width, height, coordination_number, pos
):
    walker = random_walker.RandomWalker(
        edge_list, signs, width, height, coordination_number
    )
    ...
    return (
        walker.get_step_distance(),
        walker.get_euclidean_distance(),
        original_stopped_node,
    )


def run_multiple_realizations(
    width, height, pflip, navg, navg2, coordination_number
):
    step_distances = []
    euclidean_distances = []
    stopped_nodes = []
    ...
    for _ in range(navg2):
        ...
        for _ in range(navg):
            step_distance, euclidean_distance, stopped_node = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--width", type=int, default=50, help="Width of the lattice"
    )
    parser.add_argument(
        "--height", type=int, default=50, help="Height of the lattice"
    )
    parser.add_argument(
        "--pflip",
        type=float,
        default=0.5,
        help="Probability of a negative sign",
    )
    parser.add_argument(
        "--navg", type=int, default=20, help="Number of realizations to average"
    )
    parser.add_argument(
        "--navg2",
        type=int,
        default=10,
        help="Number of times to repeat averaging",
    )
    parser.add_argument(
        "--coordination_number",
        type=int,
        default=4,
        help="Coordination number for the lattice",
    )

    args = parser.parse_args()

    avg_step_dist, avg_euclidean_dist, stopped_nodes = (
