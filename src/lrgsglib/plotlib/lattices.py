...
#
def defects_on_lattice_plot(
    sizes,
    lattices,
    ax,
    direction: str = "parallel",
    geometry: str = "squared",
    cell: str = "single",
    fit_mode: str = "lmfit",
):
    #
    ...
    for side, c in zip(sizes[::-1], cmapv):
        ...
        eigV = unravel_1d_to_2d_nodemap(
            lattices[side].eigV[0],
            lattices[side].invnode_map,
            lattices[side].syshape,
        )
        ...
        x = np.rint(
            np.linspace(-sideop // 2, sideop // 2, num=sideop) + xShiftConst
        )
        ax.plot(x, phi_plot, **kwdict)
        ax.plot(
            np.argmax(np.abs(x)) + 2, phi_plot[np.argmax(np.abs(x))] + 2, "or"
        )
        lista.append(x[np.argmax(np.abs(x)) + 2] + sideop // 2)
        print(
            np.argmax(np.abs(x)),
            x[np.argmax(np.abs(x)) : np.argmax(np.abs(x)) + 2],
        )
    #
    if fit_mode and cell != "singleXERR":
        # idx = (x < -side//3) | (x > side//3)
        x_values = (
            np.linspace(-sizes[-1] // 2, sizes[-1] // 2, num=sizes[-1])
            + xShiftConst
        )
        y_values = phi_plot0
...
def scheme_Lattice2DSquared_v02(
    ...
    for j in range(side2):
        ...
        ax.plot(
            [x[-1, j], x[-1, j] + lenExtl], [y[-1, j], y[-1, j]], **kwargsExtl
        )
    ...
    for i in range(side1):
        ...
        ax.plot(
            [x[i, -1], x[i, -1]], [y[i, -1], y[i, -1] + lenExtl], **kwargsExtl
        )

    if mode == "defects":
        ax.text(x[0, 2] + 0.5, y[0, 2] + 0.3, rf"$S$", color=cpec, **kwargsTxt)
        ax.text(x[3, 2] + 0.5, y[3, 2] + 0.3, r"$X$", color=cpec, **kwargsTxt)
        ax.text(x[2, 0] + 0.3, y[2, 0] + 0.5, r"$Z$", color=cpec, **kwargsTxt)
