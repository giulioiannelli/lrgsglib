...
def create_volume_visualization(
    ...
    for idx, (x_data, y_data, z_data, color_name, label) in enumerate(
        data_sets
    ):
        ...
        color_idx = idx % len(
            colors
        )  # Handle cases with more data_sets than colors
        fig.add_trace(
            go.Volume(
                x=X.flatten(),
                y=Y.flatten(),
        ...
        sample_idx, point_sizes, normalized_densities = (
...
def create_3d_density_volume(x_data, y_data, z_data, X, Y, Z):
    ...
    if points.shape[1] < 10:
        ...
        distances = np.linalg.norm(
            grid_points - mean_point[:, np.newaxis], axis=0
        )
    ...
    try:
        ...
        for point in sample_points.T:
            distances = np.linalg.norm(
                grid_points - point[:, np.newaxis], axis=0
            )
...
def process_density_3d_visualization(
    density,
    ...
    if density_range_filt > 1e-10:
        density_norm = (
            density_filtered - density_min_filt
        ) / density_range_filt
    else:
...
def calculate_point_densities(
    ...
    sample_points = np.column_stack(
        [x_data[sample_idx], y_data[sample_idx], z_data[sample_idx]]
    )
    ...
    if max_density - min_density > 1e-10:
        normalized_densities = (local_densities - min_density) / (
            max_density - min_density
        )
    else:
        # Random variation if all densities are the same
        normalized_densities = 0.5 + 0.2 * (
            np.random.random(len(local_densities)) - 0.5
        )

    normalized_densities = np.clip(normalized_densities, 0.0, 1.0)
    point_sizes = (
        min_point_size
        + (max_point_size - min_point_size) * normalized_densities
    )

    return sample_idx, point_sizes, normalized_densities


def create_volume_visualization_simple(
    data_sets,
    grid_resolution=30,
    grid_padding=0.2,
    colors=None,
...
def update_camera_view(
    fig, zoom_level=1.0, rotation_x=0, rotation_y=0, rotation_z=0
):
    """
...
def apply_camera_preset(fig, preset_name="default"):
    ...
    if preset_name not in CAMERA_PRESETS:
        print(
            f"Warning: Preset '{preset_name}' not found. Available presets: {list(CAMERA_PRESETS.keys())}"
        )
        preset_name = "default"
