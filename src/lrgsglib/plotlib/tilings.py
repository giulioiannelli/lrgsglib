...
def plot_hex_tiling_from_nodes(x, y, vals, ax=None, cmap="viridis"):
    ...
    for i in range(n_sample):
        for j in range(i + 1, n_sample):
            dist = np.sqrt(
                (hex_centers[i, 0] - hex_centers[j, 0]) ** 2
    ...
    polys = [base_hex + np.array([cx, cy]) for cx, cy in hex_centers_sorted]
    ...
    coll = PolyCollection(
        polys, facecolors=facecolors, edgecolors="none", antialiased=False
    )
...
def plot_octagonal_square_tiling_from_nodes(
    x,
    y,
    vals,
    ax=None,
    ...
    for i in range(n_nodes):
        ...
        if abs(offset_y + rhomb_size) < tol and abs(offset_x) < tol:
            ...
            print(
                f"Warning: Node {i} at ({node_x:.3g}, {node_y:.3g}) doesn't match expected rhomb positions"
            )
...
def plot_octagonal_square_tiling_from_pos(
    pos, vals, ax=None, cmap="viridis", edgecolors="black", linewidths=0.1
):
    """
    Extracts x and y coordinates from a position dictionary and calls
    ...
    x, y = points.T
    return plot_octagonal_square_tiling_from_nodes(
        x,
        y,
        vals,
        ax=ax,
