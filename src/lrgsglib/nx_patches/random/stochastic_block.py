...
This module provides the StochasticBlockModel class for generating
random graphs with planted community structure using the stochastic
block model (SBM).

Example
-------
>>> from lrgsglib.nx_patches.random import StochasticBlockModel
>>> sizes = [30, 30, 40]  # Three communities
>>> p_matrix = [[0.3, 0.05, 0.05], [0.05, 0.3, 0.05], [0.05, 0.05, 0.3]]
>>> sbm = StochasticBlockModel(sizes=sizes, p_matrix=p_matrix, pflip=0.1)
...
class StochasticBlockModel(RandomGraph):
    """
    ...
    **kwargs : Any
    ...
    ...             [p_out, p_in, p_out],
    ...             [p_out, p_out, p_in]]
    >>> sbm = StochasticBlockModel(sizes=sizes, p_matrix=p_matrix, seed=42)
