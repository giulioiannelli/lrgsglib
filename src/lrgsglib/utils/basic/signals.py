...
__all__ = ["bandpass_sos"]


def bandpass_sos(
    data: NDArray, lowcut: float, highcut: float, fs: float, order: int
) -> NDArray:
    """
    ...
    if highcut >= nyquist:
        raise ValueError(
            f"highcut ({highcut}) must be less than Nyquist frequency ({nyquist})"
        )
    if lowcut >= highcut:
        raise ValueError(
            f"lowcut ({lowcut}) must be less than highcut ({highcut})"
        )
    if order <= 0:
    ...
    try:
        sos = butter(
            N=order, Wn=[low_norm, high_norm], btype="band", output="sos"
        )
