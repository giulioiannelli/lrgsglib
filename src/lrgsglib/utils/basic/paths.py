...
def find_matching_files(
    search_dir: str, pattern_str: str
) -> Union[str, List[str]]:
    """
...
def extract_values_from_filenames(
    file_names: List[str],
    value_pattern: str,
    sort: bool = True,
    unique: bool = False,
...
def extract_and_sort_values(
    path: str, search_pattern: str, value_pattern: str = None, sort: bool = True
) -> NDArray:
    """
    ...
    if not re.search(r"\((?!\?:).+?\)", value_pattern):
        raise ValueError(
            "value_pattern must contain at least one capturing group."
        )
