...
def compute_normalized_linkage(
    dists: np.ndarray,
    G: nx.Graph,
    method: str = "average",
    labelList: str = "names",
...
def compute_optimal_threshold(
    linkage_matrix: np.ndarray, scaling_factor: float = 0.9
...
def dendrogram_leaf_node_colors(
    ...
    unique_cluster_colors = {
        c
        for c in colors
        if not (c.startswith("#8") or c == "grey" or c == "gray")
    }
...
def circular_layout_by_cluster(G: nx.Graph, cluster_assignment: Dict) -> Dict:
    ...
    nodes_sorted = sorted(
        G.nodes(), key=lambda node: (cluster_assignment[node], node)
    )
    n = len(nodes_sorted)
    pos = {}
