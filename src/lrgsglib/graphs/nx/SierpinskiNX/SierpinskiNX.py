"""
SierpinskiNX: Sierpinski gasket/triangle fractal graph.
...
Example
-------
>>> from lrgsglib.graphs.nx.SierpinskiNX import SierpinskiNX
>>> sg = SierpinskiNX(n=4, pflip=0.1, seed=42)
>>> sg.flip_random_fract_edges()
>>> print(f"Nodes: {sg.N}, Edges: {sg.Ne}")
"""
...
from ..FractalGraphNX.FractalGraphNX import FractalGraphNX


class SierpinskiNX(FractalGraphNX):
    """
    ...
    **kwargs
        Additional arguments passed to FractalGraphNX (pflip, seed, etc.).
    ...
    spectral_dimension : float
        Spectral dimension d_s = 2 * ln(3) / ln(5) approx 1.365 for gasket.
    ...
    def get_walk_dimension(self) -> float:
        ...
        -------
        float
            Walk dimension d_w = ln(5)/ln(2) approx 2.32 for gasket.
        """
        if self.variant == "gasket":
            return np.log(5) / np.log(2)  # approx 2.32
        elif self.variant == "carpet":
            return np.log(8) / np.log(3)  # approx 1.89
        else:
            return np.log(6) / np.log(2)  # approx 2.58
    ...
    def get_fractal_dimension(self) -> float:
        ...
        """
        if self.variant == "gasket":
            return np.log(3) / np.log(2)  # approx 1.585
        elif self.variant == "carpet":
            return np.log(8) / np.log(3)  # approx 1.893
        else:
