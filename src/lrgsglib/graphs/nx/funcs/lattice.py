...
def LatticeND_graph_FastPatch(
    dim: Tuple[int, ...], periodic: bool = False
) -> Graph:
    ...
    e_i = [
        tuple(1 if i == j else 0 for j in range(num_dimensions))
...
def LatticeND_graph_with_dilution(
    ...
    edges_to_remove = np.random.choice(
        num_edges, size=num_edges_to_remove, replace=False
    )
