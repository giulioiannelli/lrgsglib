    ...
    for pname in self.clPname:
        if os.path.exists(pname):
            try:
                ...
                print(
                    f"Warning: Could not remove Ising cluster file: {e}",
                    file=sys.stderr,
                )
...
def remove_edgl_file(self: "SignedGraphNX"):
    if os.path.exists(self.path_exp_edgl):
        try:
            ...
            print(
                f"Warning: Could not remove edgelist file: {e}", file=sys.stderr
            )
...
def remove_eigV_file(self: "SignedGraphNX"):
    if os.path.exists(self.eigVPname):
        try:
            ...
            print(
                f"Warning: Could not remove eigenvector file: {e}",
                file=sys.stderr,
            )
...
def remove_adj_file(self: "SignedGraphNX"):
    if os.path.exists(self.path_exp_adj):
        try:
            ...
            print(
                f"Warning: Could not remove adjacency file: {e}",
                file=sys.stderr,
            )
