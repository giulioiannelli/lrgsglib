...
from scipy.sparse.linalg import eigs as scsp_eigs
...
def compute_laplacian_spectrum(
    ...
    **Sparse Eigenvalue Limitation:**
    ...
    cache_type_ok = (
        getattr(self, "_spectrum_laplacian_type", None) == laplacian_type
    )
    has_cache = (
    ...
    if not is_sym and keep_sparse:
        logger.warning(
            ...
            "spectrum; falling back to dense eig (N=%d).",
            self.N,
        )
        keep_sparse = False
    ...
    if keep_sparse:
        ...
        if is_sym:
            ...
            logger.debug(
                "Computing eigenvalues with %s.eigvalsh", backend_obj.name
            )
            self.eigv = backend_obj.eigvalsh(dense_laplacian)
        else:
            # Non-symmetric (rw): general solver, real-cast & sorted
            logger.debug(
                "Computing eigenvalues with general (non-symmetric) solver"
            )
            self.eigv = _general_eigvals(dense_laplacian)

        logger.debug(
            "Eigenvalue computation complete: %d values", len(self.eigv)
        )
...
def compute_k_eigvV(
    ...
    backend_name = (
        ...
        else getattr(self, "_backend_name", "scipy")
    )
    solver = solver.lower()
    ...
    if solver == "lobpcg":
        ...
        if backend_name == "cupy":
            ...
            raise ValueError(
                "lobpcg is supported only for scipy or cupy backends."
            )
...
def compute_laplacian_spectrum_weigV(
    ...
    **Backend Selection:**
    ...
    if keep_sparse:
        ...
        lap_dense = (
            lap.astype(typf).toarray()
            if hasattr(lap, "toarray")
            else np.asarray(lap, dtype=typf)
        )
...
def compute_adjacency_spectrum_weigV(
    ...
    dense_adj = (
        ...
        else np.asarray(adj_matrix, dtype=typf)
    )
...
# get methods
#
def get_eigV_squeezed(self: "SignedGraphNX", which: int = 0):
    """
...
#
def get_eigV_binarized(self: "SignedGraphNX", which: int = 0):
    """
...
def get_sgspect_basis(
    ...
    basis_list = np.array(
        [
            get_eigV_check(self, i, reshaped=False)
...
# Layout flip helpers
#
def make_eigV_transposed(self: "SignedGraphNX"):
    self.eigV = self.eigV.T
    self._eigV_is_transposed = True


def make_eigV_column_major(self: "SignedGraphNX"):
    self.eigV = self.eigV.T
    self._eigV_is_transposed = False


def make_adj_eigV_transposed(self: "SignedGraphNX"):
    self.adj_eigV = self.adj_eigV.T
    self._adj_eigV_is_transposed = True


def make_adj_eigV_column_major(self: "SignedGraphNX"):
    self.adj_eigV = self.adj_eigV.T
    self._adj_eigV_is_transposed = False


#
# Quantum propagator methods — re-exported from shared
#
