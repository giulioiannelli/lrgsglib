...
def make_rescaled_signed_laplacian(self: "SignedGraphNX", MODE: str = "field"):
    if MODE == "field":
...
def get_node_attributes(
    self: "SignedGraphNX", attr: str = "pos", on_g: str = SG_REPR
):
...
def get_edge_mapping(
    self: "SignedGraphNX", edge, target_g, on_g: str = SG_REPR
):
    try:
    ...
    def map_values(value):
        if continuous:
            ...
            if max_val != min_val:
                ...
                norm_value = (
                    0.5  # If all values are the same, use middle of colormap
                )
...
def get_graph_neighbors(self: "SignedGraphNX", node: Any, on_g: str = SG_REPR):
