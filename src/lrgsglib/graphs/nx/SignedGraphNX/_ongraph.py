...
def flip_sel_edges(
    self: "SignedGraphNX", links: Any, name: str = "weight", on_g: str = SG_REPR
) -> None:
    neg_weights_dict = {
        (u, v): -1 * self.get_edge_data(u, v, on_g=on_g) for u, v in links
    }
    nx.set_edge_attributes(self.gr[on_g], values=neg_weights_dict, name=name)
...
def flip_random_fract_edges(
    self: "SignedGraphNX", pflip: Optional[float] = None, on_g: str = SG_REPR
) -> None:
    try:
        if pflip:
            self.pflip = pflip
            ...
            edges_to_flip = self.get_random_edges_from_set(
                self.Ne_flips, self.eset[on_g], on_g
            )
...
def unflip_all(self: "SignedGraphNX", on_g: str = SG_REPR):
    ...
    if edges_to_flip:
        ...
        pos_weights_dict = {(u, v): 1.0 for u, v in edges_to_flip}
        nx.set_edge_attributes(
            self.gr[on_g], values=pos_weights_dict, name="weight"
        )
...
def set_edges_random_normal(
    ...
    weights = {
        edge: random.normalvariate(mu, sigma) for edge in self.gr[on_g].edges()
    }
    nx.set_edge_attributes(self.gr[on_g], values=weights, name="weight")


def load_vec_on_nodes(
    self: "SignedGraphNX", vec: NDArray, attr: str, on_g: str = SG_REPR
):
...
def set_node_attributes(
    self: "SignedGraphNX", values: Any, attribute_name: Any, on_g: str = SG_REPR
):
