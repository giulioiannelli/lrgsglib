...
def get_subgraph_from_nodes(
    self: "SignedGraphNX", list_of_nodes, on_g: str = SG_REPR
):
...
def get_cluster_distribution(
    ...
    cl_len = get_eigV_cluster_sizes(self, which, binarize, val, on_g)
    dictdist_cluster_sizes = {size: cl_len.count(size) for size in set(cl_len)}
    return dictdist_cluster_sizes


def get_ferroAntiferro_regions(
    self: "SignedGraphNX", attr_str: str = "s", on_g: str = SG_REPR
):
    antiGroup = []
    ferroGroup = []
    graph = self.gr[on_g]
...
def make_connected_component_by_edge(
    ...
    value_edges = [
        (u, v)
        for u, v, attrs in self.gr[on_g].edges(data=True)
...
def handle_no_clust(self: "SignedGraphNX", NoClust: int) -> int | None:
    try:
        if self.numClustersBig == 0:
            ...
            logger.error(
                "Requested clusters exceed available. (SG_ERRMSG_NOCLUST_GTR_NCB)"
            )
            raise NoClustError(
                "Requested clusters exceed available. (SG_ERRMSG_NOCLUST_GTR_NCB)"
            )
