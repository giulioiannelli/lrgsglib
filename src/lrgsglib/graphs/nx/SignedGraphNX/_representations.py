...
from ....config.const import SG_GRAPHINT_REPR, SG_REPR
...
def upd_graph_matrices(
    self: "SignedGraphNX", format: str = "csr", on_g: str = SG_REPR
):
...
def upd_edge_sets(self: "SignedGraphNX", on_g: str = SG_REPR):
...
def upd_Degree(self: "SignedGraphNX", on_g: str = SG_REPR):
    self.degrees = list(dict(self.gr[on_g].degree).values())


def zip_reprNodes(self: "SignedGraphNX", x, on_g: str = SG_REPR):
    return dict(zip(self.gr[on_g], self.gr[x]))


def zip_reprEdges(self: "SignedGraphNX", x, on_g: str = SG_REPR):
    return dict(zip(self.gr[on_g].edges(), self.gr[x].edges()))


def upd_NodeMap(self: "SignedGraphNX", on_g: str = SG_REPR):
    self.map_node[on_g] = {
        x: {v: k for k, v in self.zip_reprNodes(x, on_g).items()}
        for x in self.graph_reprs
        if x != on_g
    }


def upd_EdgeMap(self: "SignedGraphNX", on_g: str = SG_REPR):
    graph = self.gr[on_g]
    egraph_to = lambda x: dict(zip(graph.edges(), self.gr[x].edges()))
    self.map_edge[on_g] = {
        x: {v: k for k, v in egraph_to(x).items()}
        for x in self.graph_reprs
        if x != on_g
    }
...
def upd_GraphRelabel(
    self: "SignedGraphNX", to_graph: str = SG_GRAPHINT_REPR, on_g: str = SG_REPR
) -> None:
    ...
    self.fleset[to_graph] = {
        x
        for e in self.fleset[on_g]
        if (x := self.get_edge_mapping(e, to_graph, on_g))
    }
    self.lfeset[to_graph] = {
        x
...
def upd_ReprMaps(self: "SignedGraphNX", on_g: str = SG_REPR):
    graph = self.gr[on_g]
    ngraph_to = lambda x: dict(zip(graph, self.gr[x]))
    self.map_node[on_g] = {
        x: {v: k for k, v in ngraph_to(x).items()}
        for x in self.graph_reprs
        if x != on_g
    }
    egraph_to = lambda x: dict(zip(graph.edges(), self.gr[x].edges()))
...
def upd_GraphRepr_All(
    self: "SignedGraphNX", on_g: str = SG_REPR, also_itself: bool = True
):
    if also_itself:
